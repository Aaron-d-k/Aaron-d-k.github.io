...
fn makepbm(grid : &Grid) -> String
{
    let mut outp=format!("P1\n{} {}\n",W,H);
    for row in grid.cells
    {
        for cell in row
        {
...
fn main()
{
    ...
    for _i in 1..1000
    {
        change(rx.sample(&mut myrng) as usize, ry.sample(&mut myrng) as usize, &mut grid, 0.1);
    }

    for t_s in (100..300).rev()
    {
        ...
        for _i in 1..1000000
        {
            change(rx.sample(&mut myrng) as usize, ry.sample(&mut myrng) as usize, &mut grid, beta);
        }

        fs::write(format!("stills/frame{}.pbm",t_s+10000), makepbm(&grid));

        let m = get_mag(&grid);

        println!("{rho:.3} {m:.3}");
    }
    /*let beta = 0.88137358702/2.0;
    for _j in 1..1000
    {
        for _i in 1..10000
        {
        ...
        fs::write(format!("stills/frame{}.pbm",_j+10000), makepbm(&grid));
    }*/
}